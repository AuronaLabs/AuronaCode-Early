...
const parseMarkdownBold = (text: string) => {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={index} className="font-bold text-[var(--TextHighlight)]">{part.slice(2, -2)}</strong>;
    }
    return part;
  });
};
                    ...
                    <ul className="flex flex-col gap-3 text-[13px] text-[var(--TextPrimary)] leading-relaxed mt-2">
                      {section.items.map((item, idx) => (
                        <li key={idx} className="flex gap-3 items-start">
                          ...
                          <span className="opacity-90 leading-relaxed">{parseMarkdownBold(item)}</span>
                        </li>
                      ))}
                    </ul>
                  )}
