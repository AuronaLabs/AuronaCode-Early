...
export const CHANGELOG_DATA: ChangelogEntry[] = [
  ...
  },
  {
    ...
    sections: [
      {
        title: "更新检测修复",
        description: "检查更新终于不再说谎",
        items: [
          "**根因修复**：更新端点曾被固定渠道清单挡住且短路，stable 客户端永远拿到旧清单；现按渠道拆分，stable 直连最新发布",
          "**诊断日志**：检查更新的结果与渠道写入运行日志，问题可追溯",
        ],
      },
      {
        title: "启动与 OOBE 治理",
        description: "首启不再闪烁，等待更短",
        items: [
          "**Splash 提前**：启动窗口改为最先创建加载，显著缩短splash出现前的空等",
          "**交接防闪**：主窗口先显示并由置顶的 splash 遮盖至首帧合成完成，消除 OOBE 闪现与窗口空档",
        ],
      },
      {
        title: "AI agent 执行端",
        description: "从能聊到能干活",
        items: [
          "**工具执行落地**：读文件、列目录、工作区搜索、修改文件、获取诊断、运行编辑器命令、编辑器上下文七类工具接入真实能力",
          "**多轮循环**：模型请求工具后自动执行并回填结果继续推理，单次对话最多 25 轮防失控",
          "**写操作确认**：修改文件与运行命令执行前弹出确认，拒绝后 AI 不会重试同一操作",
          "**工具卡片**：每次工具调用以卡片实时展示参数、状态与结果摘要",
        ],
      },
      {
        title: "AI 提示词专业化",
        description: "向 codex 类编码 agent 看齐",
        items: [
          "**agent 规范**：工具使用规则、先读后改、最小 diff、严格 JSON 参数等七条纪律写入系统提示词",
          "**可关闭**：设置中可停用 agent 工具执行，停用后回归纯聊天且上下文不含工具段",
        ],
      },
      {
        title: "编辑器折叠与命令化",
        description: "折叠补完最后一块拼图",
        items: [
          "**折叠接线**：折叠全部 / 展开全部接入命令与右键菜单，受真实折叠开关门控",
          "**九项能力命令化**：行注释、移动行、复制行、删除行、折叠、多光标操作全部进入命令系统，全局搜索与扩展均可触达",
        ],
      },
      {
        title: "多光标增强",
        description: "向主流编辑器的多光标看齐",
        items: [
          "**Ctrl+D 词边界**：匹配词中不再误选，多选词更精准",
          "**选区行尾加光标**：Alt+Shift+I 一键在每个选区行尾落下光标",
          "**列光标**：Ctrl+Alt+上/下在垂直方向连续加光标，到边界自动停止",
        ],
      },
      {
        title: "编辑器手感修正",
        description: "细节处见真章",
        items: [
          "**缩进跟随配置**：Tab 与智能缩进读取用户设置的制表符宽度，不再硬编码",
          "**行号栏自适应**：行号宽度随文档行数自动扩展，超大文件不再挤压",
          "**括号配对提速**：光标处配对改为局部窗口扫描，大文件编辑更跟手",
          "**多光标撤销恢复**：撤销/重做完整还原全部副光标与选区",
        ],
      },
      {
        title: "清债与工程健康",
        description: "卸下包袱轻装上阵",
        items: [
          "**死代码清除**：移除从未接线的旧兼容层实现、废弃主题表等约 1500 行死代码",
          "**脚本合并**：三份扩展构建脚本合并共享模块，消除重复与配置键冲突",
        ],
      },
    ],
  },
  {
    ...
    date: "2026-09-21",
    summary:
      "智能与生态篇：AI 侧边栏稳定性重做与体验翻新（超时分级、会话管理、重试、专业提示词、工具协议预留）、LSP 增量同步、配置原子写；扩展生态 SDK 交互真实化、VSCodeCompat 补齐与文件监视打通",
    sections: [
      {
        title: "AI 稳定性重做",
        description: "告别笼统超时，每一秒都有反馈",
        items: [
          "**超时分级**：连接 15 秒、首 token 60 秒、流式静默 90 秒、会话总时长 600 秒四层保护，失败原因可区分",
          "**状态可视化**：连接中 / 生成中 · 已用时实时显示，完成与中止记录耗时，不再干等无响应",
          "**错误分类**：首 token 超时与流式中断独立成码，六语言提示文案各归其位",
        ],
      },
      {
        title: "AI 会话与重试",
        description: "错误不再污染对话，历史不再一锅端",
        items: [
          "**多会话管理**：新建 / 切换 / 删除会话，标题自动取首问，本地最多保留 20 个会话",
          "**错误与内容分离**：失败时保留已生成文本，错误以独立卡片展示并可一键重试",
          "**上下文不再污染**：错误与中止消息不再混入下一轮模型上下文",
          "**历史升级**：多会话存储格式 v2，旧历史自动迁移，应用退出中断的消息恢复为已停止",
        ],
      },
      {
        title: "AI 提示词与工具协议",
        description: "为 agent 时代打好地基",
        items: [
          "**专业提示词**：系统提示词按代码编辑器场景重写，输出规范、代码格式与不确定性表述全部约定",
          "**工具协议预留**：完整解析并聚合 tool_calls 增量，界面给出占位提示，执行端留待后续版本",
          "**用量透传**：服务商返回的 token 用量随完成事件透传",
        ],
      },
      {
        title: "AI 界面翻新",
        description: "聊天窗口终于像个正经产品",
        items: [
          "**消息操作**：AI 回复一键复制全文，代码块带语言标签与复制按钮",
          "**状态徽章**：已停止 / 长度截断 / 工具调用占位以徽章呈现，模型与耗时随消息落档",
          "**输入升级**：输入框随内容自动增高，滚动离底时出现回到底部按钮",
          "**清空确认**：清空会话需二次确认，误触不再丢历史",
        ],
      },
      {
        title: "LSP 增量同步",
        description: "打字不再整篇传输",
        items: [
          "**增量 didChange**：单点编辑以范围变更同步语言服务器，大文件编辑传输成本大幅下降",
          "**能力协商**：客户端声明增量同步能力，按服务端协商结果自动选择增量或整文回退",
        ],
      },
      {
        title: "配置原子写",
        description: "断电也不丢配置",
        items: [
          "**原子落盘**：user-config.json 改为临时文件 + 备份让位 + 原子替换，写一半崩溃不再产生损坏文件",
          "**存储登记**：AI 聊天历史纳入本地存储所有权清单，Factory Reset 一并清理",
        ],
      },
      {
        title: "SDK 交互真实化",
        description: "官方 SDK 的弹窗不再是占位",
        items: [
          "**真实交互**：showQuickPick / showInputBox 弹出真实界面，Esc 取消、Enter 提交",
          "**权限与主题**：permissions.check 接入真实授权状态，主题模式实测返回",
          "**独立输出频道**：扩展 createOutputChannel 获得独立输出面板频道，不再借用语言服务频道",
        ],
      },
      {
        title: "VSCodeCompat 补齐与文件监视",
        description: "兼容层能力面继续拓宽",
        items: [
          "**诊断集合**：languages.createDiagnosticCollection 接入诊断中心，与 LSP 诊断互不覆盖",
          "**扩展上下文**：ExtensionContext 与 subscriptions 生命周期到位，TextDocument 三事件可用",
          "**文件监视打通**：SDK watch-workspace-path 从显式不支持变为真实监听，事件经 poll-events 送达",
        ],
      },
    ],
  },
  {
    ...
    date: "2026-09-19",
    summary:
      "性能与生态篇：扩展 SDK 增量扩充与可靠修复、编辑器增量渲染与真实折叠、AI 助手侧边栏（纯聊天）；设计收尾同步跟进：Slider 随心调节、玻璃染色光晕化、OOBE 重做、流光增强与空间管理重排",
    sections: [
      {
        title: "扩展兼容层修复",
        description: "VSCode 插件装得上，更跑得起来",
        items: [
          "**兼容层容错**：单个损坏扩展包不再中断整个目录扫描，坏包隔离并记录诊断",
          "**结构化错误码**：「兼容层未就绪」拆分为缺失/损坏等具体原因，六语言本地化提示",
          "**安装自检**：安装 VSIX 前先校验兼容层就绪，装完即坏的情况不再发生",
          "**可视化诊断**：新增扩展诊断 IPC，包加载失败与运行时失败可拉取排查",
        ],
      },
      {
        title: "扩展授权链路",
        description: "Markdown 预览「永远等待」的根因修复",
        items: [
          "**权限错误结构化**：扩展请求权限时明确区分「等待授权」与「已拒绝」",
          "**通用授权弹窗**：任何权限（工作区读取、剪贴板等）运行期触发三选一授权，授权后自动重渲染",
          "**假 watch 下线**：watch-workspace-path 不再返回假句柄，视图刷新由宿主驱动的事实写入契约",
        ],
      },
      {
        title: "SDK v1 增量扩充",
        description: "合约版本保持 1，旧扩展零影响",
        items: [
          "**大文件分块读**：read-workspace-range 按偏移分块读取，256KB 单块上限防阻塞",
          "**文件元数据**：get-workspace-file-metadata 返回大小与类型信息",
          "**交互桥**：确认对话框 / 输入框 / 快速选择列表真实弹窗（此前为占位）",
          "**剪贴板与命令**：读写系统剪贴板、execute-command 真实路由到命令注册表",
          "**事件拉取**：poll-events 拉取宿主事件；状态栏消息与通知走真实 UI",
        ],
      },
      {
        title: "编辑器增量渲染",
        description: "打字只重绘改变的那一两行",
        items: [
          "**行级 memo 命中**：搜索匹配按行预计算、诊断按行缓存、事件处理器引用恒定",
          "**高亮引用稳定化**：语法高亮刷新时未变化行复用旧 token 引用，不再全视口重渲染",
        ],
      },
      {
        title: "真实折叠",
        description: "折叠块真正从布局收起（Beta，可关闭）",
        items: [
          "**布局收缩**：折叠后隐藏行真正退出渲染与滚动高度，折叠胶囊显示隐藏行数",
          "**全坐标适配**：光标、多光标层、括号引导线、补全锚点、滚动定位与 minimap 同步映射",
          "**灰度开关**：新增 editor.trueFolding 开关，出现回归可一键回退",
        ],
      },
      {
        title: "编辑器功能靠齐",
        description: "向主流编辑器的手感看齐",
        items: [
          "**Peek 定义**：内嵌浮窗速览定义位置与行预览，点击跳转（Alt+F12 / 右键菜单）",
          "**补全文档气泡**：候选详情栏以 Markdown 渲染 LSP documentation（含代码块）",
          "**右键菜单语言项**：转到定义 / 速览 / 查找引用 / 重命名 / 格式化入菜单，宽度收窄",
          "**smoothCaret 接线**：光标平滑插值改由既有开关控制，光标闪烁与移动更顺滑",
          "**彩虹括号令牌化**：6 色括号接入主题变量，明暗分支各自调校",
        ],
      },
      {
        title: "深色高亮修正",
        description: "告别泛白，色彩各归其位",
        items: [
          "**语法色板重调**：变量/运算符/字符串/内置/类型提示五个泛白 token 全部降亮度、增辨识",
          "**Minimap 单一事实源**：小地图硬编码色板删除，改读主题变量，随明暗即时刷新",
        ],
      },
      {
        title: "Slider 随心调节",
        description: "玻璃强度不再只有三档",
        items: [
          "**连续调节**：玻璃强度 0-100 自由拖动，三档保留为锚点刻度，六轴材质连续插值",
          "**视觉重做**：玻璃轨道 + 渐变填充 + 玻璃球 Thumb（拖动微放大 + accent 光晕）",
          "**松手提交**：新增 onValueCommit，拖动流畅且落盘时机可控",
        ],
      },
      {
        title: "玻璃染色光晕化",
        description: "点缀一点颜色，而不是染一整块",
        items: [
          "**染色语言推翻**：通知与 Toast 改为中性玻璃 + 图标状态色 + 边缘光晕点缀",
          "**公式收敛**：三处硬编码染色公式统一进 glassConfig，新增光斑工具类",
        ],
      },
      {
        title: "流光与液态描边",
        description: "更明显的液态折射，更统一的选中语言",
        items: [
          "**流光增强**：specular 与 rim 系数上调约 30-50%，rim 动态扩展到 base/raised 层",
          "**主题色卡液态描边**：选中态 accent 光晕 ring，未选中玻璃弱描边告别黑边",
        ],
      },
      {
        title: "OOBE 重做",
        description: "第一眼更安静，第一步更贴心",
        items: [
          "**主题预览隔离**：三张预览卡各自渲染各自的明暗模式，浅色模式下不再全员浅色",
          "**新增编辑器偏好步骤**：字号 / 行高 / 玻璃强度首启即选，可跳过",
          "**就绪页重做**：柔和多层光晕 + 成功动效，语言步骤同步升级为色卡语言",
        ],
      },
      {
        title: "AI 助手与设置细节",
        description: "加上一点 AI 元素，收拾好每个角落",
        items: [
          "**AI 侧边栏**：可用的纯聊天（OpenAI 兼容协议、流式回复、多轮会话、本地持久化），走全局代理，密钥仅存本地",
          "**AI 设置分区**：服务商预设 / 自定义端点 / 模型配置，未配置时空态引导",
          "**空间管理重排**：环心改显示可回收占比、字号收敛四档、数字对齐",
          "**重置按钮右置**：代码智能页每行重置按钮统一到最右侧",
        ],
      },
    ],
  },
  {
    ...
    isLatest: false,
    summary:
      "玻璃语言全面落地，控件、染色、徽章与页面一次对齐；功能篇同步跟进：Git hunk 浮层、LSP 友好提示、市场下载统一走代理客户端、危险清理二次确认",
    sections: [
      {
        title: "Aurona 玻璃控件重做",
      ...
      {
        title: "编辑器与状态栏",
        description: "顺手，就在这些小地方",
        items: [
          "**Git hunk 浮层**：点击 gutter 标记即弹出该块 diff，增删行按语义色染色，Esc 或点击外部收起",
          "**LSP 状态友好化**：语言服务未安装时状态栏不再报错误码，点一下直达工具链面板安装",
          "**状态即时刷新**：安装或卸载语言服务后，状态栏图标立即跟上，无需重启",
        ],
      },
      {
        title: "安全与可靠",
        description: "说到做到，操作有底线",
        items: [
          "**市场下载统一客户端**：插件下载改走与更新检查相同的 Rust 客户端，设置里的代理承诺即刻覆盖市场",
          "**下载护栏**：新增体积上限与 URL 协议校验，SHA-256 供应链防线保持不变",
          "**危险清理二次确认**：清理用户配置或工作区前先确认一次，写清后果再动手",
        ],
      },
    ],
  },
  {
    ...
    sections: [
      {
        title: "Aurona 玻璃效果（多轴材质）",
        description: "玻璃不再只调透明度",
        items: [
          "**三档吸附 Slider**：轻透 / 均衡 / 醇厚三档刻度式选择，替代原下拉框，档位所见即所得",
          "**多轴材质令牌**：新增投影深度、背景饱和度、rim 高光基准与边框亮度四轴，三档各有体系，dark/light 独立调校",
          "**卡片去塑料**：所有玻璃卡片常驻顶部受光渐变、内侧亮缘与分档投影，边界色随档位联动，告别「死平塑料感」",
          "**双开关职责模型**：Aurona 玻璃效果管静态材质（长什么样），Aurona 流光动效管动态效果（动不动，幅度随档位缩放）",
        ],
      },
      {
        title: "Aurona 流光动效（Beta）",
        description: "更接近液态折射的动态层",
        items: [
          "**背景光域深化**：光域提亮增加景深感，主体光域 hue 微流动，色相场呼吸不再单调",
          "**浮层折射带**：specular 顶缘高光带之外新增第二条斜向折射带，rim 补左右缘微反射，浮层 hover 时高光加速响应",
          "**动画全门控**：流光关闭时背景光域与浮层流动完全停止，reduced-motion 全降级，光域 ≤5 层红线保持",
        ],
      },
      {
        title: "i18n 开发工具链",
        description: "翻译维护从手工对照到工具保障",
        items: [
          "**结构校验**：key 树 diff（六语言缺失/多余键）、占位符一致性检查进 CI，结构错误即失败",
          "**硬编码扫描**：扫描全部源码字符串字面量中的中文并输出 file:line 清单，杜绝 SourceControl 式漏网",
          "**插桩向导**：一条命令向六语言写入翻译并自动定位嵌套位置，LF 写盘保序",
        ],
      },
      {
        title: "Fliuno 重设计与抽象",
        description: "双模式行为一致，悬浮窗更干净",
        items: [
          "**共享组件抽象**：搜索行、scope 工具行、结果分组、搜索编排与键盘导航抽取为共享模块，两处行为永远一致",
          "**三序一致**：悬浮窗补齐「展示顺序 = 键盘导航顺序 = Enter 执行顺序」模型（原键盘索引与可视分组顺序脱节）",
          "**视觉收紧**：搜索行 56→52px、结果行密度提升、图标容器缩小、scope 筛选降噪、footer 弱化为图例",
          "**提示词减短**：占位符六语言统一减短为「搜索一切…」，前缀教学移交 footer",
        ],
      },
      {
        title: "空间管理 Bento 重设计",
        description: "一眼看懂占用在哪、什么能删",
        items: [
          "**总览三卡**：总占用大数字 + 「可安全清理」高亮卡（一键清理缓存/日志）+ 纯 CSS 占比环（按组扇区着色）",
          "**分类卡网格**：核心数据 / 扩展 / 工具链 / 缓存 / 日志 / 其他各一卡，可清理组带危险描边、数据类低强调",
          "**功能全保留**：全部 10 个清理目标、退出清理开关与打开目录原样保留，只重排不删功能",
        ],
      },
      {
        title: "通知收敛与信任一致性",
        description: "弹窗不再花，供应链更可信",
        items: [
          "**通知视觉收敛**：Toast 与通知中心统一为透明底彩色图标 + 左缘 3px 状态色条，状态色只用四令牌",
          "**无哈希拒装**：官方市场下载缺少 SHA-256 校验值时拒绝安装；本地 sideload 不受影响",
          "**代理言行一致**：市场安装失败提示附带「市场下载不走代理」线索，设置页范围说明同步",
          "**测试回补**：窗口恢复钳制与更新代理偏好抽取纯函数并补 12 项单测",
        ],
      },
    ],
  },
  {
    ...
    date: "2026-09-17",
    summary:
      "V0.4.3 聚焦网络代理、窗口体验与全局材质升级：新增三档网络代理设置并覆盖更新检查与工具链下载；窗口尺寸与位置跨会话记忆、首启自动最大化；OOBE 语言选择改为双列网格；控件圆角统一到新三档体系并重设计色彩主题选择器；流光升级为液态玻璃流光；修复内置扩展预览与卸载问题，Fliuno 补齐分组渲染与搜索偏好持久化",
    sections: [
      {
        title: "稳定性修复",
        description: "内置扩展与菜单溢出的顽疾清零",
        items: [
          "**扩展预览修复**：内置扩展预览不再报「WASM 组件无效」，纯 JS 扩展在兼容层未就绪时给出明确提示而非模糊报错",
          "**卸载修复**：VSIX 落盘的扩展可正常卸载，不再显示「未找到」",
          "**菜单溢出**：Marketplace 与 Fliuno 菜单项溢出改为左右箭头滚动，坚决不出现滚动条",
          "**Git 面板全量国际化**：20 处硬编码文案接入 i18n，六语言全量补齐",
          "**空态统一**：大纲 / 调试 / 通知空态对齐 Git 标杆（发光 logo、单行文案），移除多余徽标",
        ],
      },
      {
        title: "窗口与启动体验",
        description: "VSCode 同款的窗口记忆",
        items: [
          "**窗口状态记忆**：尺寸与位置跨会话恢复，显示器边界自动钳制，拔掉显示器后不至于整窗丢失，异常自动回退最大化",
          "**首启最大化**：首次启动窗口自动最大化显示，过渡无闪烁",
          "**OOBE 语言网格**：语言选择改为双列网格，语言再多也不显冗长，最矮窗口不溢出",
        ],
      },
      {
        title: "网络代理",
        description: "更新检查与工具链下载不再受网络卡壳",
        items: [
          "**三档代理**：跟随系统（默认）/ 自定义 / 不使用代理，设置页新增「网络」分类",
          "**即时生效**：自定义代理即时应用，无需重启；更新检查与下载全程走代理",
          "**范围明示**：覆盖更新检查与语言服务器 / 运行时等工具链下载，Marketplace 扩展下载暂不经过代理",
        ],
      },
      {
        title: "设计系统统一",
        description: "圆角三档体系与玻璃色卡",
        items: [
          "**圆角统一**：控件按高度分档归入新大圆角体系（大圆角 / 圆角 / 胶囊），焦点环语言与输入框一致",
          "**色彩主题选择器重设计**：玻璃质感色卡 + 顶缘高光带预览窗，强调色描边与光晕选中态",
          "**设置补全**：新增平滑滚动、Fliuno 大小写 / 正则默认值等设置项，清理孤儿项",
        ],
      },
      {
        title: "液态玻璃流光",
        description: "iOS 26 方向的全局材质升级",
        items: [
          "**有机光场**：五层光域双速度组漂移呼吸 + 极缓旋转的锥形色相场，告别「几个圆在平移」",
          "**浮层液态材质**：下拉 / 弹层玻璃获得顶缘 specular 高光带与边缘折射 rim 描边，材质微流动",
          "**性能红线**：纯 GPU 合成、动效降级、拟物强度三档联动、光域数量受控",
        ],
      },
      {
        title: "Fliuno 升级",
        description: "视觉统一与搜索偏好",
        items: [
          "**搜索框统一**：移除强调色图标徽章，与全局输入规范一致",
          "**分组结果**：命令 / 文件 / 设置 / 符号分组 sticky 标题，Modal 与工作台布局对齐",
          "**搜索偏好持久化**：大小写敏感与正则模式写入设置，Modal 与工作台共享，弹层搜索同样生效",
          "**算法基线**：前缀路由、字段加权、模糊边界与高亮区间建立单元测试基线，防抖统一 120ms",
        ],
      },
    ],
  },
  {
    ...
    date: "2026-09-15",
    summary:
      "V0.4.2 第一批更新聚焦界面语言、终端与全局质感：新增德语、意大利语与日语并修复切换语言后部分界面不刷新的问题，语言选择框改为各语言自名显示；终端配色完整接入主题并支持强调色即时跟随，暗色 ANSI 调色板补全；全局输入框统一为 Marketplace 玻璃质感设计；八套主题深色模式表面分域不再浑浊；编辑器补齐词删除、搜索预填、平滑滚动等 VSCode 体感细节",
    sections: [
      {
        title: "界面语言",
        description: "三语新增、切换即刷与自名显示",
        items: [
          "**新增德语 / 意大利语 / 日语**：全部界面文案全量翻译，OOBE 与设置中即时生效",
          "**切换即时刷新**：修复切换语言后设置侧栏、菜单、命令面板等处仍显示旧语言的问题",
          "**语言自名显示**：语言选择下拉以各语言母语呈现（简体中文永远显示「简体中文」）",
          "**防漏译守护**：新增结构性测试，任何语言缺少翻译键都会在测试阶段报错",
        ],
      },
      {
        title: "终端重设计",
        description: "配色完整跟随主题，背景回归卡片",
        items: [
          "**强调色即时跟随**：切换主题强调色时终端配色立即重建，不再需要重开终端",
          "**暗色 ANSI 补全**：暗色模式下 12 色调色板独立调校，输出不再刺眼或发灰",
          "**背景回归卡片**：移除终端独立背景色层，直接显示面板卡片表面，去除双重内边距",
        ],
      },
      {
        title: "全局输入框统一",
        description: "Marketplace 玻璃质感成为全局规范",
        items: [
          "**统一焦点语言**：所有输入框聚焦时呈现淡灰高亮边框与 14% 光晕，玻璃质感一致",
          "**圆角体系对齐**：设置搜索、Fliuno 搜索等搜索框同步 Marketplace 大圆角",
          "**三档尺寸语义**：行内小字段、常规字段与搜索框各归其位，共享同一焦点规范",
        ],
      },
      {
        title: "深色主题优化",
        description: "八套主题表面分域，暖色不再浑浊",
        items: [
          "**编辑器表面分域**：每套主题的深色模式拥有同色相低饱和编辑器底色，不再共享同一冷蓝黑",
          "**暖色主题背景层次**：rose / coral / amber 深色渐变拉开明度层次，消除脏感",
          "**玻璃质感恢复**：深色 heavy 档白通道上调，表面区分度回归",
        ],
      },
      {
        title: "编辑器体感",
        description: "向 VSCode 的操作手感再进一步",
        items: [
          "**词删除**：Ctrl+Backspace / Ctrl+Delete 按词删除，行首行尾正确处理换行，可一步撤销",
          "**搜索预填**：选中文本后 Ctrl+F 自动带入搜索词并立即执行搜索",
          "**平滑滚动**：翻页、Ctrl+End、搜索跳转等大幅滚动改为 120ms 缓动，打字保持瞬时",
          "**当前行 gutter 指示**：行号条当前行显示强调色侧标",
          "**多光标防错**：多光标下的行操作降级为主光标执行并提示，不再产生错位光标",
        ],
      },
      {
        title: "代码结构",
        description: "大文件拆分，为后续迭代减负",
        items: [
          "**编辑器提交路径抽取**：统一编辑提交（撤销入栈 + 增量写盘 + 多光标批量）抽为独立 Hook",
          "**语法高亮器拆分**：Rust 端行级高亮引擎从 editor.rs 拆至 highlight.rs，职责单一",
        ],
      },
    ],
  },
  {
    ...
    date: "2026-09-14",
    summary:
      "V0.4.1 是一次编辑器基础手感与性能收敛版本：撤销系统全面打通并修复落盘语义错误，按键编辑改为精确增量写盘，光标跟随滚动、词级跳转、翻页与括号自动补对全部就位，查找替换在当前文件完整闭环，多光标编辑落地可用，IME 合成输入手感三处修正；同时实装 Git Gutter 行标记、WASM 沙箱基准优雅降级，并集中修复多项稳定性问题",
    sections: [
      {
        title: "撤销与多光标",
        description: "撤销栈全面打通，多光标编辑一步还原",
        items: [
          "**撤销系统打通**：打字、IME 提交、删除、换行、缩进、行操作与替换全部进入撤销栈，连续打字自动合并为一步",
          "**多光标编辑落地**：Alt+Click 添加光标、Ctrl+D 选中下一个匹配，批量输入、退格、删除与换行一次提交",
          "**一步撤销**：多光标批量编辑以单一复合操作入栈，撤销一步还原全部光标改动",
          "**选中即替换**：修复选中内容后输入或回车不会替换选区的问题",
        ],
      },
      {
        title: "查找替换闭环",
        description: "当前文件查找替换语义与体验修正",
        items: [
          "**正则捕获组**：正则替换支持 $1..$99、$& 与 $$ 展开，字面量模式原样插入",
          "**替换后推进**：「替换」完成后自动定位到下一个匹配，连续替换不再中断",
          "**落盘正确性**：撤销、重做与全部替换改为按区间替换写盘，修复内容错误追加的问题",
        ],
      },
      {
        title: "IME 输入手感",
        description: "合成输入的预览、按键与候选定位修正",
        items: [
          "**预览跟随光标**：拼音等合成预览改为绝对定位渲染在光标插入点，行中输入不再错位",
          "**候选按键不再被劫持**：合成中的方向键、回车与 Esc 优先交给输入法候选窗，不再被补全面板拦截",
          "**候选锚点精确化**：输入法候选框与自动补全面板按实际前缀文本宽度定位，宽字符不再漂移",
        ],
      },
      {
        title: "Git Gutter 实装",
        description: "编辑器行号条显示 Git 变更标记",
        items: [
          "**行级变更标记**：编辑器行号条右侧显示新增（绿）、修改（蓝）与删除（红角标）标记",
          "**自动刷新**：文件保存、Git 操作与仓库切换后自动重算变更，防抖合并并发请求",
        ],
      },
      {
        title: "性能基准修复",
        description: "WASM 沙箱基准恢复可用并优雅降级",
        items: [
          "**复用已安装扩展**：WASM 基准优先使用开发机基准包，正式环境下自动复用任意已安装的 WASM 扩展",
          "**缺失时优雅跳过**：无可用扩展包时显示「已跳过」状态并附说明，不再计为失败",
        ],
      },
      {
        title: "编辑器性能与响应",
        description: "按键路径零整文传输，光标永远可见",
        items: [
          "**精确增量写盘**：所有按键编辑（退格、删除、换行、缩进、行操作）只把差异区间传给内核，大文件按键不再整文序列化",
          "**光标跟随滚动**：光标移动、打字、撤销与替换后自动滚动至光标可见，纵向行级对齐、横向按文本宽度定位",
          "**选区高亮渲染减负**：无关行的选区测量不再触发多余的 DOM 布局读取",
        ],
      },
      {
        title: "编辑器导航与自动补对",
        description: "补齐高频导航键与括号引号配对",
        items: [
          "**词级跳转**：Ctrl+←/→ 按词移动光标，配合 Shift 可扩选，支持 Unicode 标识符",
          "**快速定位**：Ctrl+Home / Ctrl+End 跳到文首文尾，PageUp / PageDown 按视口行数翻页",
          "**括号引号自动闭合**：输入 ( [ { \" ' ` 自动补全配对，选中内容自动包裹，重复输入 over-type 跳过",
        ],
      },
      {
        title: "稳定性修复",
        description: "多项并发、会话与权限问题收敛",
        items: [
          "**终端会话竞态**：快速重开终端时旧会话读取线程不再误关闭同标识的新会话",
          "**文档会话归一化**：统一 Windows 路径前缀，同一文件不再产生双份编辑会话",
          "**用户配置并发写入**：设置项并发保存不再互相覆盖丢失字段",
          "**扩展卸载权限清除**：权限记录清理失败时中止卸载，避免内存与磁盘状态不一致",
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    sections: [
      {
        title: "全新首启欢迎引导（OOBE）",
        description: "对标系统级 OOBE 的首启仪式感，主程序内嵌呈现、完整复用主题设计系统",
        items: [
          "**主程序内嵌欢迎界面**：首次启动时主窗口直接呈现全屏欢迎引导，依次完成欢迎 → 语言 → 外观主题 → Aurona 账户登录（可跳过）四步，沿用与主程序统一的标题栏、玻璃材质与整套主题令牌",
          "**主题即时预览**：浅色、深色与跟随系统切换即时生效，引导层背后的整个工作台同步变换，所见即所得",
          "**零阻塞启动**：欢迎引导以置顶覆盖层呈现，底层工作台并行启动不受阻塞；完成引导即刻落入工作台，引导中关闭窗口即退出应用",
          "**高级设置可重游**：高级设置新增「欢迎引导」入口，支持一键重新运行，随时调整语言与外观偏好",
        ],
      },
      {
        title: "安装器三语选择与母语化体验",
        description: "从安装第一步起即说用户的母语，与应用内语言设置完全对齐",
        items: [
          "**安装器三语选择器**：Windows 安装程序内置语言选择界面，支持简体中文、繁體中文与 English，安装过程全程母语化",
          "**与应用内语言对齐**：安装器语言选项与应用内「界面语言」设置一一对应，选择一次、处处一致",
        ],
      },
      {
        title: "退出清理队列与生命周期治理",
        description:
          "软件关闭时前端与 WebView 先行清理，后端在窗口销毁后完成缓存清理，拒绝后台滞留",
        items: [
          "**有序退出清理**：依据退出清理开关，在所有窗口销毁后对 WebView 缓存目录执行有界等待（约 3 秒）的 best-effort 清理，先清理、后退出",
          "**运行期清理兜底**：WebView 缓存手动清理对被占用文件做有界重试，仍锁定的文件转由退出清理队列兜底删除",
        ],
      },
      {
        title: "空间管理界面重构",
        description: "对齐设置页统一规范，信息密度与视觉层次全面升级",
        items: [
          "**条目全面重塑**：移除孤立说明文本，条目新增图标与分组色点并重新排序，一眼即辨数据归属",
          "**规范完全对齐**：间距、字号、分组与滚动布局与设置分区保持一致，滚动条出现不再引发布局偏移",
        ],
      },
      {
        title: "性能优化与基准模型修订",
        description: "构建配置与运行时表现双向优化，评分更真实、跑分更公平",
        items: [
          '**Release 构建优化**：优化等级从 opt-level="s" 提升至 "3"，开发依赖改用 opt-2 平衡编译速度与运行性能',
          "**基准测试并行化**：文件系统基准测试并行执行，缩短整体跑分耗时；性能评分模型同步修订",
          "**WASM 基准优雅降级**：基准扩展包缺失时不再阻断，性能测试其余项目照常完成",
        ],
      },
      {
        title: "交互细节与一致性修复",
        description: "通知中心、Marketplace 与更新体验的集中打磨",
        items: [
          "**通知中心与菜单统一**：通知中心消息背景卡片更新，源代码管理与通知中心菜单设计统一",
          "**Marketplace 细节修复**：卡片标题排版对齐、下拉菜单防溢出、插件卡片点击蓝边消除、离线通知规范化",
          "**更新体验完善**：检查更新增加超时保护，杜绝无反馈空转；版本更新记录重组呈现，大版本族内 Pioneer 合集一卡呈现",
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    sections: [
      {
        title: "LSP 架构彻底解耦与共享公共运行时池",
        description: "告别臃肿预打包，语言服务全面演进为按需动态分发与全局环境共享架构",
        items: [
          "**主程序安装包轻量化**：剔除数十兆预捆绑 Node.js 与语言服务二进制文件，官方全套 LSP 转为 `.aurlsp` 标准市场包按需分发",
          "**共享公共运行时 (Shared Node Runtime Pool)**：所有基于 Node.js 的 LSP 服务统一共享 APPDATA 中的单套精简 Node 运行时，杜绝多服务重复打包",
          "**Steam 风格附带环境确认弹窗**：安装依赖 Node.js 的 LSP 时智能弹出极简毛玻璃弹窗，清晰提示附带安装的运行时与体积",
          "**智能语言感知与按需就绪**：打开特定语言代码文件时，编辑器智能感知并在状态栏提示未就绪，支持从市场一键下载启动",
        ],
      },
      {
        title: "异步流式传输彻底根除 4GB OOM 崩溃与卡顿优化",
        description: "全链路重构 Rust 底层下载与文件系统操作，消灭主线程假死与掉帧",
        items: [
          "**异步流式分块直写**：采用 `reqwest::Response::chunk()` 流式边下载边写入临时文件，彻底消灭大型工具链下载导致的 OOM 与堆栈溢出崩溃",
          "**后台线程池解压与递归删除**：解压及删除包含数千文件的目录均通过 `tokio::task::spawn_blocking` 转移至独立线程池，主线程卡顿彻底归零",
          "**细粒度进度与并发渲染**：实时广播已下载字节数驱动微型环形进度与流光进度条，配合 React `startTransition` 带来极致顺滑的操作体验",
        ],
      },
      {
        title: "全新非侵入式原地权限系统（零跳转）",
        description: "告别跨侧边栏跳转，实现原生极简、就地授权的权限生命周期治理",
        items: [
          "**原地一键安全授权**：插件首次需要读取文档或工作区时，在原地面板直接展示拟物盾牌授权卡片，提供「立即授权 (始终允许)」与「仅本次允许」按钮，点击原地秒级生效并即时刷新，绝不跳转至 Marketplace 侧边栏",
          "**卸载即刻权限销毁**：扩展或语言服务卸载时，自动扫描并彻底清除其所有工作区权限与会话权限并立即持久化，再次安装需重新申请授权",
          "**iOS 风格安全弹窗**：支持微拟物毛玻璃浮层弹窗，为敏感操作提供明确清晰的授权确认",
        ],
      },
      {
        title: "Aurona Marketplace 现代微拟物设计与交互重塑",
        description: "重塑扩展市场视觉与交互体验，对齐大厂顶级设计标准",
        items: [
          "**沉浸式微拟物毛玻璃详情页**：高对比度 Hero 区域、大图标立体拟物阴影、一键复制 Identifier 胶囊、全功能 Markdown README 深度渲染、分级权限卡片、4 格特性网格、时间轴更新日志与交互式 5 星评价体系",
          "**Git 同款菜单按钮与防溢出排版**：探索与已安装采用与 Git 面板完全一致的独立纯文字按钮，分类下拉选择器自适应弹性排版，彻底解决狭窄侧边栏下的宽度溢出与换行问题",
          "**消除点击白边**：全面去除 SubTabs 点击时产生的刺眼白色外框，融入极简微拟物滑块动效",
          "**设备维度真实下载量统计**：废弃账号依赖，基于实际文件分发请求进行电脑维度统计与格式化（k/w 展示），未登录用户下载同样准确计数",
          "**更新与卸载独立三态**：存在可用新版本时，提供醒目的「更新至 vX.Y.Z」主按钮与红雾「卸载」副按钮，支持自由选择",
        ],
      },
      {
        title: "官方扩展生态矩阵升级 (v0.1.2) 与 SDK v1.2.0",
        description: "官方扩展与原生插件开发工具链全方位增强",
        items: [
          "**Planner 任务计划 v0.1.2**：控制栏重构为现代双行排版（第一行「全部/待办/已完成」状态分段网格，第二行「优先级选择器 + 搜索框」两栏布局），告别拥挤；适配全新原地安全授权",
          "**Markdown 预览 v0.1.2**：适配全新原地安全授权，升级三语国际化 (i18n)，优化大文档实时渲染防抖与内存管理",
          "**AuronaSDKHost v1.2.0 升级**：新增 `aurona.permissions` 原生权限检测与请求 API，为第三方插件提供类型安全的权限交互契约",
        ],
      },
      {
        title: "空间管理数据主权完善与发布质量保证",
        description: "可视化管理本地工具链物理占用，全链路自动化质量检验",
        items: [
          "**工具链与运行时独立统计**：空间管理面板新增「语言服务与共享运行时」分类，精准呈现物理占用并支持一键安全清理",
          "**程序本体 100% 纯净**：所有扩展包、语言服务、公共运行时与配置统一保存在系统 AppLocalData 目录，严格保持安装目录只读",
          "**全链路自动化门禁**：Rust 核心编译、TypeScript 严格类型检查与 Vitest 全量单元测试套件全部自动化通过",
          "**包完整性端到端校验**：WASM 扩展归档与 LSP 语言服务包通过严格的结构完整性与哈希一致性检验",
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    sections: [
      {
        title: "扩展包物理安装、卸载与版本更新闭环",
        description: "打通 WASM 扩展从市场到本地运行时的全生命周期管理",
        items: [
          "**扩展真实安装引擎**：Rust 核心新增 `extensions_install` 命令，接收二进制包并写入系统应用本地数据目录，自动完成解包注册与运行时热重载",
          "**扩展安全卸载与清理**：新增 `extensions_uninstall` 命令，支持物理删除已安装包、注销注册表项并安全重置沙箱运行时，杜绝内存残留",
          "**版本更新检测与升级**：市场详情页自动比对本地已安装版本与远程最新版本，支持一键无缝升级",
          "**内置扩展防篡改保护**：内置核心扩展受系统级写保护，禁止被外部包覆盖安装或非法卸载",
        ],
      },
      {
        title: "Aurona Account 登录认证与市场登录态互通",
        description: "基于标准 OIDC/PKCE 协议构建安全的桌面端账户认证链路",
        items: [
          "**PKCE S256 授权码登录**：底层集成标准 OAuth 2.0 / OIDC 流程，通过系统默认浏览器完成授权，基于本地动态 TCP Loopback 安全接收授权回调",
          "**JWKS 验签与安全凭证管理**：自动拉取 OIDC 元数据与 JWKS 公钥验签 ID Token，Refresh Token 安全持久化存储，支持自动静默刷新与一键注销",
          "**市场服务认证无缝对接**：通过账户 Access Token 自动换取市场专用 JWT，打通扩展一键星标收藏与真实用户评分评价提交",
        ],
      },
      {
        title: "页面切换状态保活（Tab Keep-Alive）架构重构",
        description: "攻克历史页面切换导致内容丢失的顽疾，保障多任务切换体验",
        items: [
          "**内置视图常驻挂载**：设置中心、扩展市场、关于面板与 Fliuno 独立工作区等内置页面采用常驻保活挂载机制，彻底消除切换标签页导致的表单重置或滚动位置丢失",
          "**Visibility 视觉与事件隔离**：非活动标签页自动置于隐藏状态并禁用事件捕获，兼顾零损耗保活与极低的前台渲染开销",
          "**多任务无缝穿梭**：在代码编辑、设置微调与市场浏览之间自由往返切换，所有页面数据与交互状态即刻呈现",
        ],
      },
      {
        title: "设置中心快速定位与平滑高亮导航（Deep Link Anchor）",
        description: "打通全局搜索与具体设置项之间的无缝锚点跳转链路",
        items: [
          "**设置快速定位传参 (`openSettings`)**：支持从 Fliuno 全局搜索、快捷命令或外部模块直接传参指定目标设置分类与配置项 ID",
          "**平滑滚动与高亮聚焦**：页面加载后自动平滑滚动至对应配置卡片并触发柔和聚焦高亮动画，清晰引导用户视线",
          "**全 10 大分类枚举约束**：基于 EventBus 统一治理设置导航事件流，杜绝路径失效与无效跳转",
        ],
      },
      {
        title: "通知中心多维分类与非重要通知免打扰",
        description: "告别通知一股脑堆积，提供清爽可控的信息中枢",
        items: [
          "**多分类筛选胶囊**：通知面板新增分类过滤控制器，支持按全部、系统、扩展、更新等维度清晰归类查看",
          "**非重要通知静音设置**：设置中心新增通知管理卡片，支持开启「仅将非重要通知记录于通知中心，不弹出 Toast 弹窗」",
          "**Toast 停留时长自定义**：支持用户个性化调节弹窗停留秒数，提供更舒适的阅读节奏",
        ],
      },
      {
        title: "Aurona Marketplace 菜单重构与开发者头像系统",
        description: "全面对齐专业源码管理布局，提升开发者专属归属感",
        items: [
          "**探索与已安装子菜单重构**：与 Git 源码管理子菜单风格完全对齐，移除已安装扩展的数字角标干扰",
          "**刷新按钮对齐与质感升级**：升级为与 SourceControl 完全一致的 16px 拟物刷新按钮，带 Tooltip 提示与平滑加载动效",
          "**插件开发者专属卡片**：详情页侧边栏引入 42px 大尺寸头像与官方认证徽标卡片，清晰呈现发布者归属",
          "**微型头像贯穿全流程**：Hero 区域发布者胶囊、元数据清单行及市场列表卡片均统一接入 `AccountAvatar` 微型头像（13-16px）",
        ],
      },
      {
        title: "Marketplace 全量 API 对接与生态闭环",
        description: "解除市场闭合，全面接入本地与远程市场端点全部特性",
        items: [
          "**探索页实时防抖检索**：200ms 防抖响应输入、分类联动筛选、顶部一键刷新旋钮与在线/离线自适应指示",
          "**安装包与版本历史下载**：支持一键触发 `.aurx` 真实安装包下载，并在更新日志页提供历史版本列表下载",
          "**星标收藏交互**：Hero 区集成星标胶囊按钮，实时切换收藏状态并同步收藏计数值",
          "**WASI 0.2 权限清单卡片**：详情页全面展示声明权限列表，按正常与敏感级别色彩划分并提供详细说明",
          "**安全评分与审计仪表**：展示安全评分动态进度条（如 98/100）及沙箱安全审计通过说明",
          "**用户评价与讨论区**：提供交互式 1-5 星打分表单、评论提交、用户评价列表、开发者官方回复与有用点赞功能",
        ],
      },
      {
        title: "0.4.x 系列视觉质感跃升与细节打磨",
        description: "从启动页到权限弹窗的全面审美进化",
        items: [
          "**0.4.x 全新 Splash 启动背景**：换装 0.4.x 系列专属全新启动画面与极简光影壁纸，带来更沉浸的第一视觉体验",
          "**iOS 极简毛玻璃权限弹窗**：采用官方 `GlassContainer` 浮动层构建，居中排版与纯净「仅本次允许 / 始终允许 / 拒绝」三级按钮",
          "**语言去 Beta 标识**：移除界面语言选择中繁体中文与英文的 `(Beta)` 标记，展现更成熟稳定的产品面貌",
          "**先锋通道切换静音**：切换先锋计划开关时移除繁琐弹窗提示，操作更加清爽安静",
          "**编辑器设置增强**：新增 Minimap 代码缩略图开关与 Smooth Caret 平滑光标动画开关",
          "**Rust 核心 Quality 门禁达标**：全量修正代码格式化与 Clippy 警告，确保跨平台自动化构建 100% 绿灯",
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "**体积与性能极致精简**：扩展 WASM 体积大幅削减 > 105 KiB (`auronalabs.markdown.aurx` 降至 966 KiB)，冷启动效率提升 40%",
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        title: "可靠性与安全保存",
        items: [
          "**安全保存**：Windows 保存既有文件采用备份替换与失败回滚，写入过程异常也不会损坏原文件",
          "**标签保护**：中键关闭未保存标签也会进入确认流程，误触不再直接丢失修改",
        ],
      },
      {
        title: "终端、语言服务与架构统一",
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        title: "细节体验修缮",
        items: [
          "**资源管理器悬停革新**：去除早期遗留的蓝色悬停高亮，代之以贴合主题的玻璃色阶，选中与悬停反馈与全局材质一致",
          "**新建输入框标准化**：新建文件的输入框已全面切换为标准无边框组件，与全局控件观感统一",
        ],
      },
    ],
  },
