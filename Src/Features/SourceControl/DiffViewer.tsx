...
export function DiffViewer({ commitHash }: DiffViewerProps) {
  ...
  useEffect(() => {
    async function loadDiff() {
      try {
        ...
        const rawDiff: string = await invoke("git_diff_commit", { path: repoPath, hash: commitHash });
  ...
  const parseGitDiff = (rawText: string) => {
    ...
    for (; i < lines.length; i++) {
      ...
      if (line.startsWith("-")) {
        currentHunk.lines.push({ type: "deletion", content: line.substring(1), leftLineNum: leftLineNumber++, rightLineNum: null });
      } else if (line.startsWith("+")) {
        currentHunk.lines.push({ type: "addition", content: line.substring(1), leftLineNum: null, rightLineNum: rightLineNumber++ });
      } else if (line.startsWith(" ")) {
        currentHunk.lines.push({ type: "context", content: line.substring(1), leftLineNum: leftLineNumber++, rightLineNum: rightLineNumber++ });
      }
    }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[var(--AppBackground)] overflow-hidden">
      <div className="flex-1 overflow-y-auto aurona-scroll p-6">
        {files.length === 0 ? (
          <div className="text-[var(--TextMuted)] text-center py-10">此提交未包含支持差异对比的文件</div>
        ) : (
          <div className="flex flex-col gap-6">
            {files.map((file, fileIdx) => {
              const additions = file.hunks.reduce((acc, hunk) => acc + hunk.lines.filter((l) => l.type === "addition").length, 0);
              const deletions = file.hunks.reduce((acc, hunk) => acc + hunk.lines.filter((l) => l.type === "deletion").length, 0);

              return (
              <div key={fileIdx} className="rounded-xl border border-[var(--GlassBorder)] bg-[var(--GlassSurface)] backdrop-blur-sm overflow-hidden shadow-sm">
                <div className="px-4 py-2.5 bg-black/5 dark:bg-white/5 border-b border-[var(--GlassBorder)] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] font-bold text-[var(--TextHighlight)]">
                      {file.oldName === file.newName ? file.newName : `${file.oldName} -> ${file.newName}`}
                    </span>
                  </div>
                ...
                <div className="flex flex-col text-[13px] font-mono leading-[1.6]">
                  {file.hunks.map((hunk, hunkIdx) => (
                    <div key={hunkIdx} className="flex flex-col border-b border-[var(--GlassBorder)] last:border-b-0">
                        ...
                        <div className="w-1/2 overflow-x-auto aurona-scroll border-r border-[var(--GlassBorder)]">
                          <div className="flex flex-col min-w-max">
                            {hunk.lines.map((line, lineIdx) => {
                              ...
                              let leftBg = isContext ? "bg-transparent" : isDel ? "bg-red-500/10" : "bg-black/5 dark:bg-white/5";
                              let leftText = isContext ? line.content : isDel ? line.content : "";
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className={`px-4 flex-1 flex items-center whitespace-pre ${leftBg} ${isDel ? "text-red-600 dark:text-red-400" : "text-[var(--TextPrimary)]"}`}>
                                    {leftText || " "}
                                  </div>
                                </div>
                              );
                        ...
                        <div className="w-1/2 overflow-x-auto aurona-scroll">
                          <div className="flex flex-col min-w-max">
                            {hunk.lines.map((line, lineIdx) => {
                              ...
                              let rightBg = isContext ? "bg-transparent" : isAdd ? "bg-green-500/10" : "bg-black/5 dark:bg-white/5";
                              let rightText = isContext ? line.content : isAdd ? line.content : "";
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className={`px-4 flex-1 flex items-center whitespace-pre ${rightBg} ${isAdd ? "text-green-600 dark:text-green-400" : "text-[var(--TextPrimary)]"}`}>
                                    {rightText || " "}
                                  </div>
                                </div>
                              );
