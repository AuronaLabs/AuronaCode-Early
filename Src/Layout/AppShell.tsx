...
export function AppShell({ Children }: AppShellProps) {
  ...
  const activityItems = [
    { label: ACTIVITY_EXPLORER, Icon: Icons.Files },
    { label: ACTIVITY_SOURCE_CONTROL, Icon: Icons.Git },
  ];
  ...
  return (
    <div className="flex h-dvh w-screen flex-col bg-[var(--ColorApp)] text-[var(--ColorText)] overflow-hidden">
      <TitleBar />

      <div className="flex flex-1 min-h-0 overflow-hidden">
        <nav className="flex w-[var(--ActivityBarWidth)] shrink-0 flex-col items-center bg-[var(--ColorApp)] pt-0 pb-1 relative z-20">
        ...
        <main className="flex flex-1 min-w-0 overflow-hidden bg-[var(--ColorApp)] relative">
          <div className="absolute inset-0 h-full w-full">{Children}</div>
        </main>
      </div>

      <footer className="flex h-[var(--StatusBarHeight)] shrink-0 items-center bg-[var(--ColorApp)] px-4 text-xs text-[var(--ColorMuted)] font-medium overflow-hidden">
