  ...
  ({ icon, active = false, badge = false, className = "", title, ...props }, ref) => {
    const buttonElement = (
      <button
        type="button"
        ref={ref}
        className={`relative flex h-[calc(var(--ActivityBarWidth)-16px)] w-[calc(var(--ActivityBarWidth)-16px)] min-h-8 min-w-8 items-center justify-center rounded-control transition-[background-color,color,transform] duration-150 focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/60 ${
          active
            ? "bg-[var(--material-interactive-active)] backdrop-blur-[var(--glass-blur-raised)] text-[var(--color-text-highlight)] ring-1 ring-inset ring-[var(--border-subtle)]"
