...
export function Modal({ isOpen, onClose, title, children, footer, icon }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/30 backdrop-blur-lg p-4 animate-in fade-in duration-300">
      <div 
        className="bg-[var(--GlassSurface)] backdrop-blur-2xl border border-[var(--GlassBorder)] rounded-2xl w-full max-w-sm flex flex-col overflow-hidden animate-in fade-in zoom-in-90 slide-in-from-bottom-2 duration-300 ease-out" 
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col p-5 gap-3">
          {title && (
            <div className="flex items-center justify-between">
              <h3 className="text-[16px] font-semibold text-[var(--TextHighlight)] flex items-center gap-2">
                {icon}
                {title}
              </h3>
              {onClose && (
                <button 
                  onClick={onClose}
                  className="p-1 rounded-md text-[var(--TextMuted)] hover:bg-black/5 dark:hover:bg-white/10 hover:text-[var(--TextHighlight)] transition-colors"
                >
