...
const aurxPath = join(root, "src-tauri", "resources", "extensions", "aurona.planner.aurx");
...
function main() {
  ...
  if (manifest.packageVersion !== 1 || manifest.id !== "aurona.planner") {
  ...
  console.log(`aurona.planner.aurx 校验通过: ${aurxPath} (${data.length} bytes)`);
}
