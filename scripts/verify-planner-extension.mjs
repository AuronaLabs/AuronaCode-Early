...
const aurxPath = join(root, "MarketplacePackages", "auronalabs.planner.aurx");
...
function main() {
  ...
  if (manifest.packageVersion !== 1 || manifest.id !== "auronalabs.planner" || manifest.publisher !== "auronalabs") {
  ...
  console.log(`auronalabs.planner.aurx 校验通过: ${aurxPath} (${data.length} bytes)`);
}
